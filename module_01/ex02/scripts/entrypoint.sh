...
echo "🚀 Starting PostgreSQL for Exercise 02 - Removing duplicate rows from customers table"
...
if [ "$table_count" -eq 0 ]; then
    ...
    # First execute the customers table creation script to join all tables
    echo "📊 Step 1: Creating customers table by joining all data_202* tables..."
    psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f /scripts/customers_table.sql
    
    # Now remove duplicates from the customers table
    echo "🧹 Step 2: Removing duplicate rows from customers table..."
    psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -f /scripts/remove_duplicates.sql
