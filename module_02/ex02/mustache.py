...
def create_price_box_plot(data, zoom_to_main_range=False):
    ...
    if zoom_to_main_range:
        ...
        ax.set_xlim(
            max(lower_bound, data["price"].min()),
            min(upper_bound, data["price"].quantile(0.95)),
        )

    ax.set_xlabel("price")
    ax.set_title(
        "Box Plot: Price Distribution of Purchased Items",
        fontsize=14,
        fontweight="bold",
    )
...
def create_basket_box_plot(data):
    ...
    ax.boxplot(
        user_avg_basket["avg_basket_price"],
        vert=False,
        patch_artist=True,
        boxprops=dict(facecolor="lightblue", alpha=0.7),
        medianprops=dict(color="red", linewidth=2),
    ...
    ax.set_xlabel("price")
    ax.set_title(
        "Box Plot: Average Basket Price per User", fontsize=14, fontweight="bold"
    )
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    return fig
