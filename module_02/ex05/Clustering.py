...
def extract_customer_features():
    ...
    try:
        ...
        data['engagement_rate'] = data.apply(
            lambda x: x['active_days'] / x['customer_lifespan_days'] if x['customer_lifespan_days'] > 0 else 1, 
            axis=1
        )
        data['purchase_intensity'] = data.apply(
            lambda x: x['total_purchases'] / x['active_days'] if x['active_days'] > 0 else x['total_purchases'], 
            axis=1
        )
...
def create_customer_segments(data):
    ...
    clustering_features = data[[
        ...
        'days_since_last_purchase', 'engagement_rate', 'purchase_intensity'
    ]].copy()
    ...
    def assign_customer_segment(row):
        ...
        if spending >= spending_high and frequency >= frequency_high and recency <= recency_recent:
            return 'Platinum Customer'
        
        # Gold: High spending OR High frequency + Medium recency
        elif (spending >= spending_high or frequency >= frequency_high) and recency <= recency_old:
        ...
        elif frequency <= data['total_purchases'].quantile(0.3) and recency <= recency_recent:
            return 'New Customer'
...
def create_four_key_visualizations(data):
    ...
    for segment in data['customer_segment'].unique():
        ...
        plt.scatter(segment_data['days_since_last_purchase']/30, 
    ...
    # Chart 3: K-Means Clustering Visualization
    plt.figure(figsize=(12, 8))
    ...
    for i in range(5):
        ...
        plt.scatter(pca_features[cluster_mask, 0], pca_features[cluster_mask, 1], 
                   c=cluster_colors[i], label=f'Cluster {i+1}', alpha=0.6, s=50)
    
    # Plot centroids
    pca_centroids = pca.transform(kmeans.cluster_centers_)
    plt.scatter(pca_centroids[:, 0], pca_centroids[:, 1], 
               c='yellow', marker='o', s=200, alpha=0.8, 
    ...
    fig.suptitle('Business Intelligence: Customer Segment Analysis', fontsize=16, fontweight='bold')
    
    # Revenue per segment
    colors_list = [segment_colors.get(seg, '#95A5A6') for seg in segment_metrics.index]
    bars1 = ax1.bar(segment_metrics.index, segment_metrics['Avg_Spent'], color=colors_list)
    ...
    # Purchase frequency
    bars2 = ax2.bar(segment_metrics.index, segment_metrics['Avg_Purchases'], color=colors_list)
    ...
    # Recency analysis
    bars4 = ax4.bar(segment_metrics.index, segment_metrics['Avg_Recency'], color=colors_list)
...
def print_segment_analysis(data):
    ...
    segment_analysis.columns = [f"{col[0]}_{col[1]}" if col[1] else col[0] for col in segment_analysis.columns]
    ...
    for segment in segment_analysis.index:
        ...
        print(f"   Revenue: ${total_segment_revenue:,.2f} ({revenue_percentage:.1f}% of total)")
        ...
        if segment == 'New Customer':
            print("   📧 EMAIL STRATEGY: Welcome series, onboarding offers, first-purchase incentives")
        elif segment == 'Inactive Customer':
            print("   📧 EMAIL STRATEGY: Win-back campaigns, special discounts, re-engagement series")
        elif segment == 'Platinum Customer':
            print("   📧 EMAIL STRATEGY: VIP treatment, exclusive offers, early access, loyalty rewards")
        elif segment == 'Gold Customer':
            print("   📧 EMAIL STRATEGY: Premium offers, upselling, loyalty program invites")
        elif segment == 'Silver Customer':
            print("   📧 EMAIL STRATEGY: Value offers, cross-selling, loyalty building")
        else:
            print("   📧 EMAIL STRATEGY: General promotions, product recommendations, engagement content")
...
def main():
    ...
    print(f"Business Segments Created: {len(segmented_data['customer_segment'].unique())}")
